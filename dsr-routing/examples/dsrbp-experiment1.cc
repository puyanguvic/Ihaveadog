...
main (int argc, char *argv[])
{
  ...
  for (uint32_t i = 0; i < nodes.GetN (); i ++)
    {
      Ptr<Ipv4> tempIpv4 = nodes.Get (i)->GetObject<Ipv4> ();
      for (uint32_t j = 1; j < tempIpv4->GetNInterfaces (); j ++)
        {
          Ptr<Channel> temp = tempIpv4->GetNetDevice (j)->GetChannel ();
          TimeValue delay;
          temp->GetAttribute ("Delay", delay);
          uint32_t matric = delay.Get().GetMicroSeconds ();
          tempIpv4->SetMetric(j, matric);
        }
    }
